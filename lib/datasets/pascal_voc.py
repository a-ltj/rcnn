...
import matplotlib.pyplot as plt		#画pr曲线的
import pylab as pl
...
#    self._classes = ('__background__',  # always index 0
#                     'insulator', 'vibration_damper', 'defect', 'nest','tower','vibration_damper_rust')
  ...
  def _load_pascal_annotation(self, index):
    ...
    format.
    """   
    filename = os.path.join(self._data_path, 'Annotations\\'+index + '.xml')#路径在这    
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    pl.xlabel('Recall')			#画PR曲线的
    pl.ylabel('Precision')
    plt.grid(True)
    pl.ylim([0.0, 1.05])		#可以升高y坐标 试试    1.05
    pl.xlim([0.0, 1.0])
